...
envir_data <- c()
folder <- 'C:/Users/StevenSummey/Desktop/VB_Share/May_29_2024_1519/home/vboxuser/PhysiCell/output/'
# zipped_xml_names <- grep('\\.xml$', unzip('C:/Users/StevenSummey/Desktop/VB_Share/output.zip', 
#                                           list=TRUE)$Name, 
#                          ignore.case=TRUE, value=TRUE)
# unzip('C:/Users/StevenSummey/Desktop/VB_Share/output.zip', files=zipped_xml_names)
# comb_tbl <- rbindlist(lapply(zipped_xml_names,  
#                              function(x) cbind(fread(x, sep=',', header=TRUE,
#                                                      stringsAsFactors=FALSE),
#                                                file_nm=x)), fill=TRUE ) 
# files <- data.frame(outputs = unique(comb_tbl$file_nm))
LoadSim <- function(path) {
  ...
  ## GET FIRST XML
  data_param <<- read_xml(paste0(folder, "output00000001.xml"))
  measured_data <<-
    data_param %>%
    xml_find_all("//label") %>%
    xml_text()
...
# loadGammaNames <- function(GammaNames) {
#   CELL_OBJECT <- GET_CELL_OBJECT(0,firstTime = TRUE)
...
#   GeneToKeep <- paste0("GeneGamma", rownames(FACTOR_LIST))
#   GeneToKeepNewName <-
#     paste0(paste0(FACTOR_LIST$Var2, "->"), FACTOR_LIST$Var1)
#   for (i in 1:length(GammaNames)) {
#     if (GammaNames[i] %in% GeneToKeep) {
#       GammaNames[i] <- GeneToKeepNewName[match(GammaNames[i], GeneToKeep)]
#     }
#   }
#   
#   return(GammaNames)
...
server <- function(input, output, session) {
  ...
  LoadSim("C:/Users/StevenSummey/Desktop/VB_Share/May_29_2024_1519/home/vboxuser/PhysiCell/output/")
...
# myBookmarks<-"./"
# shinyOptions(myBookmarksPath=myBookmarks)
# my_save_interface <- function(id, callback) {
#   stateDir <- file.path(myBookmarks, "shiny_bookmarks", id)
...
# }
#
# shinyOptions(save.interface = my_save_interface, load.interface = my_load_interface)


sApp <- shinyApp(ui, server, enableBookmarking = "server")
#runApp(sApp)

# profvis({
# },prof_output = "./SHINY_APP")
# profile<-profvis(prof_input="./SHINY_APP/prof.Rprof")
# htmlwidgets::saveWidget(profile,"./export.html")
