---
title: "Potential Particulate/Turbidity Assessment via iD5 SpectraMax Microplate Reader (DRAFT)"
...
(1) Kinetic Protocol (15-minute span, 8 measurements total, reduction: mean)
      
      (i) 230nm (sensitive to protein conformation)
        -> A230 is a valid and convenient structural probe to determine thermodynamic stability and unfolding kinetics of proteins with many potential applications.[1]
     (ii) 260nm (nucleic acid detection)
    (iii) 280nm (protein detection)
     (iv) 650nm (practical turbidity wavelength)
      (v) 980nm (turbidity interferes with pathcheck protocol at 900-1000nm)
     
    
(2) Dilutions from 1x to 16x for all seven samples
      (i) WFI H2O (baseline sample)
     (ii) FFB2 (baseline buffer)
    (iii) FFB2 + PS20 (0.005%, sample buffer)
     (iv) DP 0% (high presence of particulates)
      (v) DP 0.001% (moderate presence of particulates)
     (vi) DP 0.005% (no particulates observed in aliquot)
    (vii) DI2 (turbidity detection, 14DEC2023)
   
   
(3) Kinetic Protocol performed at 5 temperatures (C) for thermal aggregate detection
      (i) 25
     (ii) 35
    (iii) 45
     (iv) 55
...
## OD Values at Increasing Temperatures: Overview
...
## Comparability to Release Testing 
...
pDiff_report <- read.csv("C:/Users/StevenSummey/OneDrive - Aruna Bio/Characterization/Turbidity/pDiff_reportTable.csv",
                         header = TRUE)
colnames(pDiff_report) <- c("Measurement","DP 0", "DP 0.001", "DP 0.005", "percent CV")
pDiff_report %>%
  kbl() %>%
  kable_styling()
