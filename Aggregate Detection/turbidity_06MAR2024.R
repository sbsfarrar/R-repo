...
fc_file = read.csv("C:/Users/StevenSummey/OneDrive - Aruna Bio/Characterization/Turbidity/FC_Counts_Cleaning.csv")
fc_counts = fc_file
fc_counts = data.frame(fc_counts, id = rep("water",nrow(fc_counts)))

plot_fc <- ggplot(fc_counts) + 
  geom_line(aes(x = Sample, y=Large.Particulate, group=id),size=0.8,alpha = 0.5) + 
  geom_point(aes(x=Sample, y=Large.Particulate)) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5))
plot_fc

plot_fc <- ggplot(fc_counts) + 
  geom_line(aes(x = Sample, y=Total.Counts, group=id),size=0.8,alpha = 0.5) + 
  geom_point(aes(x=Sample, y=Total.Counts)) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5))
plot_fc
...
fc_file = read.csv("C:/Users/StevenSummey/OneDrive - Aruna Bio/Characterization/Turbidity/FC_Counts_05MAR2024.csv")
#fc_counts = fc_file[which(fc_file$Sample != "FFB2+PS20"),1:4]
fc_counts <- fc_file[,1:7]
...
x1 =  factor(fc_counts$Temp, levels=c("4","25","35","45","55","65","25C INC", "4C END"))

plot_fc <- ggplot(fc_counts, aes(x=x1, y=Total.Particles, group=Sample,color=Sample))
...
plot_fc <- plot_fc + theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=0.5)) +
  ylab("Particles/mL") + xlab("Temperature") + labs(title = "Total Particles at Increasing Temperatures, by Sample",
                                                    subtitle = "05MAR2024")
plot_fc


plot_fc <- ggplot(fc_counts, aes(x=x1, y=Total.Particulate, group=Sample,color=Sample))
plot_fc <- plot_fc + geom_line(size = 0.8, alpha = 0.5) + geom_point(size = 1.5)
plot_fc <- plot_fc + theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=0.5)) +
...
plot_fc_small <- ggplot(fc_counts, aes(x=x1, y=Small.Particulate, group=Sample,color=Sample))
...
plot_fc_large <- ggplot(fc_counts, aes(x=x1, y=Large.Particulate, group=Sample,color=Sample))
...
fc_counts$PercentofParticles <- fc_counts$Total.Particulate/(fc_counts$Total.Particles)
...
plot_fc_smallperc <- ggplot(fc_counts, aes(x=x1, y=PercentSmall, group=Sample,color=Sample))
...
plot_fc_largeperc <- ggplot(fc_counts, aes(x=x1, y=PercentLarge, group=Sample,color=Sample))
...
plot_fc_particleperc <- ggplot(fc_counts, aes(x=x1, y=PercentofParticles, group=Sample,color=Sample))
plot_fc_particleperc <- plot_fc_particleperc + geom_line(size = 0.8, alpha = 0.5) + geom_point(size = 1.5)
plot_fc_particleperc <- plot_fc_particleperc + theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=0.5)) +
  ylab("Percent of Particulates Counted") + xlab("Temperature") + labs(title = "Percent of Total Particles at Increasing Temperatures, by Sample",
                                                                       subtitle = "05MAR2024")
plot_fc_particleperc
#-----------------------------------------------------------------------------#
#-----------------------------------------------------------------------------#
#-----------------------------------------------------------------------------#
#-----------------------------------------------------------------------------#
...
turb = read.csv("C:/Users/StevenSummey/OneDrive - Aruna Bio/Characterization/Turbidity/Turbidity_05MAR2024_TEMP.csv")
turb["OD"][turb["OD"] < 0] <- 0
#turb_samp <- turb[grepl('DP',turb$Sample, fixed=T),]
turb_samp <- turb[which(turb$Sample != "H2O"),]
turb_samp <- turb_samp[which(turb_samp$Sample != "FFB"),]
# turb_DI2 <-turb[grepl('DI2',turb$Sample, fixed=T),]
# turb_buff <- turb[grepl('FFB2',turb$Sample, fixed=T),]
x1 =  factor(fc_counts$Temp, levels=c("4","25","35","45","55","65","25C INC", "4C END"))

turb_230 = turb_samp[which(turb_samp$Wavelength == "230"),setdiff(1:5,3)]
turb_230 = turb_230[which(turb_230$Dilution == "2"),-which(colnames(turb_230) == "Dilution")]
turb_230_melt <- melt(turb_230,id= c("Sample","Temp"))

plot_230 <- ggplot(turb_230_melt, aes(x=x1, y=value, group=Sample,color=Sample))
...
turb_260 = turb_samp[which(turb_samp$Wavelength == "260"),setdiff(1:5,3)]
turb_260 = turb_260[which(turb_260$Dilution == "2"),-which(colnames(turb_260) == "Dilution")]
turb_260_melt <- melt(turb_260,id= c("Sample","Temp"))

plot_260 <- ggplot(turb_260_melt, aes(x=x1, y=value, group=Sample,color=Sample))
...
turb_280 = turb_samp[which(turb_samp$Wavelength == "280"),setdiff(1:5,3)]
turb_280 = turb_280[which(turb_280$Dilution == "2"),-which(colnames(turb_280) == "Dilution")]
turb_280_melt <- melt(turb_280,id= c("Sample","Temp"))

plot_280 <- ggplot(turb_280_melt, aes(x=x1, y=value, group=Sample,color=Sample))
...
turb_380 = turb_samp[which(turb_samp$Wavelength == "380"),setdiff(1:5,3)]
turb_380 = turb_380[which(turb_380$Dilution == "2"),-which(colnames(turb_380) == "Dilution")]
turb_380_melt <- melt(turb_380,id= c("Sample","Temp"))

plot_380 <- ggplot(turb_380_melt, aes(x=x1, y=value, group=Sample,color=Sample))
plot_380 <- plot_380 + geom_line(size = 0.8, alpha = 0.5) + geom_point(size = 1.5)
plot_380 <- plot_380 + theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=0.5)) +
  ylab("OD at 380nm") + xlab("Temperature")+ labs(title = "OD Values at Increasing Temperatures, by Sample",
 subtitle = "380nm")
plot_380

turb_650 = turb_samp[which(turb_samp$Wavelength == "650"),setdiff(1:5,3)]
turb_650 = turb_650[which(turb_650$Dilution == "2"),-which(colnames(turb_650) == "Dilution")]
turb_650_melt <- melt(turb_650,id= c("Sample","Temp"))

plot_650 <- ggplot(turb_650_melt, aes(x=x1, y=value, group=Sample,color=Sample))
...
plot_230_280 <- ggplot(turb_ratio, aes(x=x1, y=ratio, group=Sample,color=Sample))
...
grid_plot <- ggarrange(plot_230,plot_260,plot_280,plot_380, plot_650, plot_230_280, ncol=2,nrow=3,
...
turb = read.csv("C:/Users/StevenSummey/OneDrive - Aruna Bio/Characterization/Turbidity/Turbidity_05MAR2024.csv")
turb["OD"][turb["OD"] < 0] <- 0
...
turb_230 = turb_230[which(turb_230$Dilution == "1"),-which(colnames(turb_230) == "Dilution")]
comb_turb_230 <- cbind(turb_230, totalParticulate = fc_counts$PercentofParticles, 
                       smallParticulate = fc_counts$PercentSmall,
                       largeParticulate = fc_counts$Large.Particulate)
#turb_230_melt <- melt(turb_230,id= c("Sample","Temp"))
# 
# plot_230 <- ggplot(turb_230_melt, aes(x=Temp, y=value, group=Sample,color=Sample))
# plot_230 <- plot_230 + geom_line(size = 0.8, alpha = 0.5) + geom_point(size = 1.5)
...
# plot_230
#from https://finchstudio.io/blog/ggplot-dual-y-axes/
max_first  <- max(comb_turb_230$OD)   # Specify max of first y axis
max_second <- max(comb_turb_230$largeParticulate) # Specify max of second y axis
min_first  <- 0#min(comb_turb_230$OD)   # Specify min of first y axis
min_second <- 0#min(comb_turb_230$largeParticulate) # Specify min of second y axis
...
p2 <- ggplot(comb_turb_230,aes(x=x1)) + 
  geom_line(aes(y=OD,alpha=0.5,group=Sample,color=Sample)) + 
        geom_point(aes(y=OD),color="red",alpha=0.5) + 
  geom_line(aes(y=inv_scale_function(largeParticulate, scale, shift),alpha=0.5,group=Sample,color=Sample)) + 
        geom_point(aes(y=inv_scale_function(largeParticulate,scale,shift)),color="green",alpha=0.5) + 
  scale_y_continuous("OD",#limits=c(min_first,max_first), 
                                   sec.axis = sec_axis(~scale_function(., scale, shift),name="Large Particulate")) + 
  scale_color_brewer(palette="Set1") + guides(alpha="none") 
  # ggtitle("Large Particulate and OD")
